...
def makePage(logo,Mname,Mcode,ip_list,timenow,robot_setting,gaussian_setting,sixmic_info):
    ...
    if not take_picture.getPicture():
        final_web_page = web_page.format(logo,Mname,Mcode,ip_list,timenow,robot_setting,gaussian_setting,
            resolution_setting,'','','拍照失败！',sixmic_info)
    else:
        picture_status = file_version.getFileSizeandTime("files/foreward.jpg")
        final_web_page = web_page.format(logo,Mname,Mcode,ip_list,timenow,robot_setting,gaussian_setting,
            resolution_setting,
            picture_status[0],
            picture_status[1],'拍照成功！',sixmic_info)
