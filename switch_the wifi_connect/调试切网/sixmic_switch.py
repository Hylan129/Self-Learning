...
def kill_server():
    res = subprocess.Popen('taskkill /f /im AlarmCenterMonitorService.exe',shell=True,
...
@sixmin_ini
def wifi_switch(ssid,password):
    try:
        command_aiui = wifi_config(ssid,password)
        send(command_aiui)
    except Exception as e:
        with open('err.txt','a') as code:
            code.write("sixmic_switch:" + str(e) + '\n')
    finally:
        port_close()
    return "六麦网络切换成功！"
